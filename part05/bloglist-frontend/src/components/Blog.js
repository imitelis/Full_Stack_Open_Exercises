...
const Blog = ({ blog, user, removeBlog, likeBlog }) => {
  ...
  return(
    <div className='blog'>
      {blog.title} by {blog.author} <button onClick={() => setBlogVisible(!blogVisible)}>{blogVisible ? 'hide' : 'show'}</button> <br />
      {blogVisible ?
        <div>
        url: {blog.url} <br />
        likes: {blog.likes} <button onClick={updatelikesBlog}>like</button> <br />
