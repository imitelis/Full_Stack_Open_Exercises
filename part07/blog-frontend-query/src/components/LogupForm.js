...
const LogupForm = ({ user, newUserMutation }) => {
  ...
  const handleErrorResponse = (error, username) => {
    if (error?.response?.status === 500) {
      ...
      notificationDispatch({
        type: "RED_NOTIFICATION",
        payload: `user log up failed: username (${username}) already exists`,
      });
  ...
  const handleLogup = async (event) => {
    ...
    try {
      if (!name || !username || !password) {
        notificationDispatch({
          type: "RED_NOTIFICATION",
          payload: `error: username (${username}), name (${name}) and password (*) are required`,
        });
        ...
        notificationDispatch({
          type: "RED_NOTIFICATION",
          payload: `error: username (${username}) minlength must be of three characters`,
        });
        setUsername("");
      } else {
        const userObject = {
        ...
        newUserMutation.mutate(userObject, {
          onSuccess: () => {
            notificationDispatch({
              ...
              payload: `successfully logged up ${username}! now you can log in`,
            });
            navigate("/login");
          },
          onError: (error) => {
