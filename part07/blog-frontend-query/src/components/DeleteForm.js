        ...
        await loginService.login({
          ...
          if (currentUser) {
            removeUserMutation.mutate(deletedUser, {
              onSuccess: () => {
                notificationDispatch({ type: "GREEN_NOTIFICATION", payload: `${user.username} user successfully deleted`})
              },
              onError: (error) => {
          ...
          navigate('/login');
            ...
            notificationDispatch({ type: "RED_NOTIFICATION", payload: `error: wrong credentials or non-existing user`})
          }
          } catch (error) {
            handleErrorResponse(error, user);
          }
