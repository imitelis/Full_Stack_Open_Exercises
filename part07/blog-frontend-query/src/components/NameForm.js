...
const NameForm = ({ user, users, updateUserMutation, innerRef }) => {
    ...
    const handleErrorResponse = (error, user) => {
      if (error?.response?.status === 500) {
        ...
        notificationDispatch({ type: "RED_NOTIFICATION", payload: `error: wrong credentials or session expired, ${user.username} please log and try again`})
    ...
    const changeName = async (returnedForm) => {
      try {
        if (!returnedForm.password || !returnedForm.newName) {
          notificationDispatch({ type: "RED_NOTIFICATION", payload: `error: password (*) and new name (${returnedForm.newName}) are required`})
        } else if (user.username === returnedForm.newName) {
          notificationDispatch({ type: "RED_NOTIFICATION", payload: `error: name (${user.name}) and new name (${returnedForm.newName}) are equal`})
        } else {
          ...
          if (currentUser.username === user.username) {
            ...
            try {
              ...
              updateUserMutation.mutate(updatedUser, {
                onSuccess: () => {
                  notificationDispatch({ type: "GREEN_NOTIFICATION", payload: `${user.username} name successfully updated`})
                },
                onError: (error) => {
                  handleErrorResponse(error, user);
                }
